/**
 * jQuery Form Builder Plugin Copyright (c) 2009 Mike Botsko, Botsko.net LLC
 * (http://www.botsko.net)
 * http://www.botsko.net/blog/2009/04/jquery-form-builder-plugin/ Originally
 * designed for AspenMSM, a CMS product from Trellis Development Licensed under
 * the MIT (http://www.opensource.org/licenses/mit-license.php) Copyright notice
 * and license must remain intact for legal use
 */
(function($) {
	...
	return this.each(function() {
		...
		case 'input_text':
		    ...
		    break;
		case 'file':
		    appendFileInput(values, options, required);
		    break;
		case 'textarea':
	    ...
	    // Add fiel upload field
	    var appendFileInput = function(values, options, required) {
		field += '<div class="frm-fld"><label>' + opts.messages.label + '</label>';
		field += '<input class="fld-title form-control" id="title-' + last_id + '" type="text" value="' + values + '" /></div>';
		help = '';
		appendFieldLi(opts.field_types.file.label, field, required, help);
	    };
